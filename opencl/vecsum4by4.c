				...
				cl_mem d_vsum, cl_mem d_v1, cl_mem d_v2, cl_int nels, cl_event init_evt)
{
	...
	err = clSetKernelArg(vecsum_k, i++, sizeof(nhex), &nhex);
	ocl_check(err, "set vecsum arg_nhex", i-1);
...
int main(int argc, char *argv[])
{
	...
	if (nels & 15)	// Vale solo per potenze di 2 (per 4 gli ultimi due bit sono 0 se è 4)
	{
		fprintf(stderr, "nels must be multiple of 16\n");	// Non usare modulo.
		exit(1);
	}
	...
	cl_kernel vecsum_k = clCreateKernel(prog, "vecsum4x4", &err);
