...
cl_event matinit(cl_kernel matinit_k, const size_t *lws, cl_command_queue que, 
	cl_mem d_A, cl_int nrows, cl_int ncols)
{
	const size_t gws[] = {round_mul_up(ncols, lws[0]), 
...
cl_event imginit(cl_kernel imginit_k, const size_t *lws, cl_command_queue que,
	cl_mem d_I, cl_int nrows, cl_int ncols)
{
	...
	ocl_check(err,"set imginit arg dI");

	err= clEnqueueNDRangeKernel(que, imginit_k, 2, NULL, gws, lws, 0, NULL, &imginit_evt);
	ocl_check(err,"enqueue imginit");
	return imginit_evt;
}

cl_event transpose(cl_kernel transpose_k, const size_t *lws, cl_command_queue que, 
	cl_mem d_T, cl_mem d_I, cl_int nrows_T, cl_int ncols_T, cl_event init_evt)
{
	const size_t gws[] = {round_mul_up(ncols_T, lws[0]), 
		round_mul_up(nrows_T, lws[1])};
	cl_event trans_evt;
	cl_int err;
  ...
  err = clSetKernelArg(transpose_k, i++, sizeof(d_I), &d_I);
	ocl_check(err, "set tranpose arg_dI", i-1);
...
{
	if (argc <= 4) {
		fprintf(stderr, "specify number of rows and columns and lws 0,1\n");
		exit(1);
	}
  ...
  cl_platform_id plat_id = select_platform();
	...
	cl_program prog = create_program("transpose_img4.ocl", ctx, dev_id);
	cl_int err;
	...
	cl_event init_evt, copy_evt, imginit_evt, trans_evt, read_evt;	// copy_evt = inefficiente
	...
	trans_evt = transpose(transpose_k, lws, que, d_T, d_I, nrows_T, ncols_T, copy_evt);
  ...
  const double trans_bw_gbs = 2.0*memsize/1.0e6/runtime_init_ms;
	const double read_bw_gbs = memsize/1.0e6/runtime_read_ms;

	printf("init: %dx%d int, lws[%ld*%ld], in %gms: %g GB/s %g GE/s\n",
		nrows_A, ncols_A, lws[0], lws[1], runtime_init_ms, init_bw_gbs, nrows_A*ncols_A/1.0e6/runtime_init_ms);
  printf("copy: %dx%d int, lws[%ld*%ld], in %gms: %g GB/s %g GE/s\n",
		nrows_T, ncols_T, lws[0], lws[1], runtime_copy_ms, copy_bw_gbs, nrows_T*ncols_T/1.0e6/runtime_copy_ms);
	printf("imginit: %dx%d int, lws[%ld*%ld], in %gms: %g GB/s %g GE/s\n",
		nrows_A, ncols_A, lws[0], lws[1], runtime_imginit_ms, imginit_bw_gbs, nrows_A*ncols_A/1.0e6/runtime_imginit_ms);
	printf("transpose: %dx%d int, lws[%ld*%ld], in %gms: %g GB/s %g GE/s\n",
		nrows_T, ncols_T, lws[0], lws[1], runtime_init_ms, init_bw_gbs, nrows_T*ncols_T/1.0e6/runtime_init_ms);
	printf("read: %dx%d int, lws[%ld*%ld], in %gms: %g GB/s %g GE/s\n",
		nrows_T, ncols_T, lws[0], lws[1], runtime_read_ms, read_bw_gbs, nrows_T*ncols_T/1.0e6/runtime_read_ms);
