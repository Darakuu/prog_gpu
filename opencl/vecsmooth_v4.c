	...
	cl_mem d_vsmooth, cl_mem d_v1, cl_int nels, cl_event init_evt)
{
	...
	ocl_check(err, "set vecsmooth arg", i-1);
	err = clSetKernelArg(vecsmooth_k, i++, sizeof(nquarts), &nquarts);
	ocl_check(err, "set vecsmooth arg", i-1);
...
int main(int argc, char *argv[])
{
	...
	if (nels & 3 )
	{
		fprintf(stderr, "nels must be a multiple of 4\n");
		exit(1);
	}
  ...
  cl_platform_id plat_id = select_platform();
	...
	cl_kernel vecsmooth_k = clCreateKernel(prog, "vecsmooth_v4", &err);
	...
	const double smooth_bw_gbs = 3.5*memsize/1.0e6/runtime_smooth_ms; // Legge 6 elementi e scrive 4 => 10 letture, 4 scritture => 14/4=3.5~
