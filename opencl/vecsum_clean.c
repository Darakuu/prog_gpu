...
int main(int argc, char *argv[])
{
  ...
  cl_platform_id plat_id = select_platform();
	...
	err = clGetKernelWorkGroupInfo(vecsum_k, dev_id, CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE, 
  ...
  // Non eseguire mai kernel mentre il buffer è mappato, undefined behaviour, idem se mappo su un ReadBuffer.
	cl_int * h_vsum = clEnqueueMapBuffer(que,d_vsum,CL_FALSE,CL_MAP_READ, 0,memsize,1,&sum_evt, &read_evt , &err);
