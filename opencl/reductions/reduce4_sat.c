...
cl_event reduce4(cl_kernel reduce4_k, cl_command_queue que,
	...
	cl_int lws_, cl_int nwg_ , cl_event init_evt)
{
	const size_t gws[] = { nwg_ * lws_ };
...
int main(int argc, char *argv[])
{
	if (argc <= 3) 
  {
		fprintf(stderr, "specify number of elements, lws and nwg\n");
		exit(1);
	}
  ...
  const int nwg = atoi(argv[3]);
	const size_t memsize = nels*sizeof(cl_int); //Not floats because of precision errors
  const size_t lws_nwg_mem = lws*nwg*sizeof(cl_int);  // (cont.) float sum is not associative!!
  ...
  if (lws & 3) ocl_check(CL_INVALID_VALUE, "lws");
	...
	ocl_check(err, "create kernel reduce4_sat");
	...
	d_v2 = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_HOST_READ_ONLY, lws_nwg_mem, NULL, &err);
  ...
  int nquarts = nels/4;
  reduce_evt[1]=reduce4(reduce4_k, que, d_v2, d_v1, nquarts, lws, nwg, reduce_evt[0]);  // riduco ad lws * nwg elementi
  reduce_evt[2]=reduce4(reduce4_k, que, d_v1, d_v2, (lws*nwg)/4, lws, 1, reduce_evt[1]);  // riduco ulteriormente ad lws elementi
  reduce_evt[3]=reduce4(reduce4_k, que, d_v2, d_v1, lws/4, 1, 1, reduce_evt[2]);  // concludo

	int risultato;
	err = clEnqueueReadBuffer(que, d_v2, CL_TRUE, 0, sizeof(risultato), &risultato, 1, reduce_evt + 3, &read_evt);
...

  {
    ...
    printf("reduce0: %d int in %gms: %g GB/s %g GE/s\n",
      nels, runtime_pass_ms, pass_bw_gbs, nels/1.0e6/runtime_pass_ms);
  }
  {
    const double runtime_pass_ms = runtime_ms(reduce_evt[2]);
    const double pass_bw_gbs = (lws_nwg_mem+lws_mem)/1.0e6/runtime_pass_ms;
    printf("reduce1: %d int in %gms: %g GB/s %g GE/s\n",
  ...
  }
  {
    ...
    printf("reduce2: %d int in %gms: %g GB/s %g GE/s\n",
      lws, runtime_pass_ms, pass_bw_gbs, lws/1.0e6/runtime_pass_ms);
  }

	const double runtime_reduction_ms = total_runtime_ms(reduce_evt[1], reduce_evt[4]);
	printf("reduce : %d int in %gms: %g GE/s\n",
